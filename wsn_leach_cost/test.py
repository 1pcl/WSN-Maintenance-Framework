...
from utilities import GLOBAL_CONFIG,DEFAULT_PARAM_VALUES,MODULES
...
application="animal_room"         #   electricity_meter, animal_room
...
# 打印结果
print("="*50)
print(f"选择所有模块的结果 ({len(selected_modules)}个模块)")
print("="*50)
...
# 保存结果到文件
with open("all_modules_simulation_results.txt", "w") as f:
    f.write("所有模块仿真结果\n")
    f.write("="*50 + "\n")
    f.write(f"应用场景: {application}\n")
    f.write(f"选择的模块 ({len(selected_modules)}个):\n")
